...
public class PaymentController {
    ...
    @Operation(summary = "Initiate a new payment", description = "Starts a payment transaction for a booking")
    public PaymentResponseDTO initiatePayment(@RequestBody PaymentRequestDTO request) {
        return paymentService.initiatePayment(request);
    }

    @PostMapping("/verify")
    @Operation(summary = "Verify payment status", description = "Verifies payment transaction with payment gateway")
    ...
    @Operation(summary = "Get payment details by ID", description = "Fetches details of a specific payment")
    public PaymentResponseDTO getPaymentById(@PathVariable Long paymentId) {
        return paymentService.getPaymentById(paymentId);
    }

    @GetMapping("/user/{userId}")
    @Operation(summary = "Get payments by user ID", description = "Fetches all payments made by a user")
    public List<PaymentResponseDTO> getPaymentsByUser(@PathVariable Long userId) {
        return paymentService.getPaymentsByUser(userId);
    }

    @GetMapping("/vendor/{vendorId}")
    @Operation(summary = "Get payments by vendor ID", description = "Fetches all payments received by a vendor")
