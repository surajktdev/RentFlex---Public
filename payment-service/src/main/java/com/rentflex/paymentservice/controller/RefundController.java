...
public class RefundController {
    ...
    @Operation(summary = "Initiate a refund", description = "Starts a refund process for a completed payment")
    public RefundResponseDTO initiateRefund(@RequestBody RefundRequestDTO request) {
        return refundService.initiateRefund(request);
    }

    @GetMapping("/{refundId}")
    @Operation(summary = "Get refund details by ID", description = "Fetches details of a specific refund")
    public RefundResponseDTO getRefundById(@PathVariable Long refundId) {
        return refundService.getRefundById(refundId);
    }

    @GetMapping("/transaction/{transactionId}")
    @Operation(summary = "Get refund by transaction ID", description = "Fetches refund details using the payment transaction ID")
    ...
    @GetMapping("/all")
    @Operation(summary = "Get all refunds", description = "Fetches all refund transactions in the system")
