...
public class ItemAvailabilityController {
    ...
    @PostMapping("/")
    public ResponseEntity<ItemAvailabilityResponse> setItemAvailability(@RequestBody ItemAvailabilityRequest request){
        ItemAvailabilityResponse itemAvailabilityResponse = itemAvailabilityService.setItemAvailability(request);
        return new ResponseEntity<>(itemAvailabilityResponse, HttpStatus.CREATED);

    }

    @GetMapping("/item/{itemId}")
    public ResponseEntity<ItemAvailabilityResponse> getAvailabilityByItem(@PathVariable Long itemId){
        ItemAvailabilityResponse availabilityByItem = itemAvailabilityService.getAvailabilityByItem(itemId);
        return new ResponseEntity<>(availabilityByItem, HttpStatus.FOUND);
    }

    @PutMapping("/{id}")
    public ResponseEntity<ItemAvailabilityResponse> updateAvailability(@PathVariable Long id, @RequestBody ItemAvailabilityRequest request){
        ItemAvailabilityResponse itemAvailabilityResponse = itemAvailabilityService.updateAvailability(id, request);
